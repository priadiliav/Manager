  ...
  private readonly ILogger<InMemoryLongPollingClient<TResponse>> _logger;
    ...
    _logger.LogInformation("Starting long polling client for URL: {PollingUrl}", _pollingUrl);
      ...
      try
      {
        ...
        {
          _logger.LogError(
              "Failed to receive response from long polling URL: {PollingUrl}, Status Code: {StatusCode}", _pollingUrl,
              response.StatusCode);
      ...
      {
        _logger.LogError("Error during long polling request to URL: {PollingUrl}", _pollingUrl);
      }
      finally
      {
